describe('Data Entry through the Data Collection Instrument', () => {
	before(() => {
		...
		cy.get('span.ui-button-text').contains('Create').parent().click({force: true})
		cy.get('span.ui-button-text').contains('Add instrument here').parent().click({force: true})
		cy.get('input#new_form-my_first_instrument').type('Field Types')
		cy.get('input.jqbuttonmed ui-button ui-widget ui-state-default ui-corner-all').click({force: true})
		//not correctly clicking button to create instrument
	})
